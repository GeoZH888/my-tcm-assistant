...
export default function TCMAssistant() {
  ...
  // Load data
  useEffect(() => {
    const ak = storage.get('tcm_api_key', '');
    const su = storage.get('tcm_supabase_url', '');
    const sk = storage.get('tcm_supabase_key', '');
    setApiKey(ak);
    setSupabaseUrl(su);
    setSupabaseKey(sk);
    if (su && sk) { supabase.configure(su, sk); setSyncStatus('connected'); loadFromSupabase(); }
    else { setDocuments(storage.get('tcm_documents', [])); setHealthLogs(storage.get('tcm_health_logs', [])); }
  }, []);
      ...
      <header style={s.header}>
        <div style={s.logo}>
          ...
          <div>
            ...
            <div style={s.logoSub}>{syncStatus === 'connected' ? '🟢 云同步' : '📱 本地'}{apiKey ? ' · AI' : ''}</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {supabase.isConfigured() && <button style={s.iconBtn} onClick={loadFromSupabase}><Icons.Refresh /></button>}
          <button style={s.iconBtn} onClick={() => setShowSettings(true)}><Icons.Settings /></button>
        </div>
      </header>
        ...
        <div style={s.overlay} onClick={() => setShowSettings(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 11, color: '#666', marginTop: 4 }}>获取：<a href="https://platform.deepseek.com" target="_blank" style={{ color: '#3B82F6' }}>platform.deepseek.com</a></p>
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ ...s.label, display: 'flex', alignItems: 'center', gap: 6 }}><Icons.Database /> Supabase 云同步</label>
              <input type="text" value={supabaseUrl} onChange={e => setSupabaseUrl(e.target.value)} placeholder="Project URL" style={{ ...s.input, marginBottom: 6 }} />
              <input type="password" value={supabaseKey} onChange={e => setSupabaseKey(e.target.value)} placeholder="anon public key" style={s.input} />
              <p style={{ fontSize: 11, color: '#666', marginTop: 4 }}>获取：<a href="https://supabase.com" target="_blank" style={{ color: '#3B82F6' }}>supabase.com</a> → Settings → API</p>
            </div>
            <div style={s.modalActions}>
              <button style={s.cancelBtn} onClick={() => setShowSettings(false)}>取消</button>
              <button style={s.confirmBtn} onClick={saveSettings}>保存</button>
            </div>
          </div>
        </div>
      )}
